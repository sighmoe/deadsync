...
// --- Initialization ---
pub fn initialize_game_state(win_w: f32, win_h: f32, audio_start_time: Instant) -> GameState {
    info!(
...
fn spawn_arrows(state: &mut GameState, rng: &mut impl Rng) {
    ...
    if target_16th_index > state.last_spawned_16th_index {
        ...
        for i in (state.last_spawned_16th_index + 1)..=target_16th_index {
            ...
            if !should_spawn {
                trace!(
                    "Skipping spawn for index {} (type {:?}) due to difficulty {}",
                    i,
                    note_type,
                    config::DIFFICULTY
                );
            ...
            let dir: ArrowDirection =
                if config::DIFFICULTY >= 4 && state.last_spawned_direction.is_some() {
                    ...
                    *available_dirs
                        .as_slice()
                        .choose(rng)
                        .unwrap_or(&ALL_ARROW_DIRECTIONS[0])
                } else {
        ...
        column_arrows.retain(|arrow| {
            ...
            if beat_diff > miss_window_beats {
                info!(
                    ...
                    arrow.target_beat,
                    current_beat,
                    beat_diff * seconds_per_beat * 1000.0,
                    config::MISS_WINDOW_MS
                );
                missed_count += 1;
                false
            } else {
...
pub fn draw(
    ...
    let _arrow_texture = assets
        .get_texture(TextureId::Arrows)
        .expect("Arrow texture missing");
    let now = Instant::now();
    ...
    for column_arrows in game_state.arrows.values() {
        for arrow in column_arrows {
            ...
            if arrow.y < (0.0 - config::ARROW_SIZE)
                || arrow.y > (game_state.window_size.1 + config::ARROW_SIZE)
            {
                continue;
            }

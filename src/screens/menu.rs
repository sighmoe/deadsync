...
pub fn draw(
    ...
    let font = assets.get_font(FontId::Wendy).expect("Main font not loaded"); // Using Wendy for menu
    ...
    let space_for_menu_top_y = logo_bottom_edge_y + 40.0 * (window_height / config::UI_REFERENCE_HEIGHT); // Increased padding
    let space_for_menu_bottom_y = window_height - 20.0 * (window_height / config::UI_REFERENCE_HEIGHT);
    ...
    for (index, option_text_str) in config::MENU_OPTIONS.iter().enumerate() {
        ...
        let neutral_baseline_y = first_item_default_baseline_y + (index as f32 * menu_item_baseline_spacing);
        ...
        let text_width_pixels = font.measure_text_normalized(option_text_str) * current_effective_scale;
        ...
        renderer.draw_text(
            ...
            x_pos, actual_baseline_y_for_draw_text,
            current_color,

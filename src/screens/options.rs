...
pub fn draw(
    ...
    let font = assets.get_font(FontId::Wendy).expect("Main font not loaded"); // Use Wendy for Options screen
    ...
    let title_baseline_y = 80.0 * (window_height / config::UI_REFERENCE_HEIGHT) + (font.metrics.ascender * title_effective_scale);
    
    trace!("Options title: baseline_y={:.1}, scale={:.2}", title_baseline_y, title_effective_scale);
    renderer.draw_text(
        device, cmd_buf, font, title,
        center_x - title_width_pixels / 2.0, title_baseline_y,
    ...
    let placeholder_width_pixels = font.measure_text_normalized(placeholder_text) * placeholder_effective_scale;
    let placeholder_baseline_y = window_height / 2.0 + (font.metrics.ascender * placeholder_effective_scale);

    trace!("Options placeholder: baseline_y={:.1}, scale={:.2}", placeholder_baseline_y, placeholder_effective_scale);
    renderer.draw_text(
        device, cmd_buf, font, placeholder_text,
        center_x - placeholder_width_pixels / 2.0, placeholder_baseline_y,
        config::MENU_NORMAL_COLOR,
    ...
    let help_baseline_y = window_height - (60.0 * (window_height / config::UI_REFERENCE_HEIGHT)) + (font.metrics.ascender * help_effective_scale);

    trace!("Options help: baseline_y={:.1}, scale={:.2}", help_baseline_y, help_effective_scale);
    renderer.draw_text(
        device, cmd_buf, font, help_text,
        center_x - help_width_pixels / 2.0, help_baseline_y,
        config::MENU_NORMAL_COLOR,

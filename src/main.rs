...
const W1_WINDOW_MS: f32 = 22.5; const W2_WINDOW_MS: f32 = 45.0; const W3_WINDOW_MS: f32 = 90.0; const W4_WINDOW_MS: f32 = 135.0;
const MAX_HIT_WINDOW_MS: f32 = 180.0; const MISS_WINDOW_MS: f32 = 200.0;
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum NoteType { Quarter, Eighth, Sixteenth }
const TARGET_TINT: [f32; 4] = [0.7, 0.7, 0.7, 0.5];
const ARROW_TINT_QUARTER: [f32; 4] = [1.0, 0.6, 0.6, 1.0]; const ARROW_TINT_EIGHTH: [f32; 4] = [0.6, 0.6, 1.0, 1.0]; const ARROW_TINT_SIXTEENTH: [f32; 4] = [0.6, 1.0, 0.6, 1.0];
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum Judgment { W1, W2, W3, W4, Miss }
const FLASH_COLOR_W1: [f32; 4] = [0.2, 0.7, 1.0, 0.9]; const FLASH_COLOR_W2: [f32; 4] = [1.0, 0.8, 0.2, 0.9]; const FLASH_COLOR_W3: [f32; 4] = [0.2, 1.0, 0.2, 0.9]; const FLASH_COLOR_W4: [f32; 4] = [0.8, 0.4, 1.0, 0.9];
const FLASH_DURATION: Duration = Duration::from_millis(120);
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
enum ArrowDirection { Left, Down, Up, Right }
const ARROW_DIRECTIONS: [ArrowDirection; 4] = [ ArrowDirection::Left, ArrowDirection::Down, ArrowDirection::Up, ArrowDirection::Right,];

#[derive(Debug, Clone)]
struct TargetInfo { x: f32, y: f32, direction: ArrowDirection }
#[derive(Debug, Clone)]
struct Arrow { x: f32, y: f32, direction: ArrowDirection, note_type: NoteType, target_beat: f32 }
#[derive(Debug, Clone, Copy)]
struct FlashState { color: [f32; 4], end_time: Instant }
...
#[repr(C)]
#[derive(Debug, Clone, Copy)]
struct PushConstantData { model: Matrix4<f32>, color: [f32; 4], uv_offset: [f32; 2], uv_scale: [f32; 2] }

#[derive(Debug, Hash, PartialEq, Eq, Clone, Copy)]
#[repr(u32)]
pub enum VirtualKeyCode { Left, Down, Up, Right, Escape, }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let file = File::open(audio_path).map_err(|e| format!("Failed to open audio file {:?}: {}", audio_path, e))?;
    info!("Audio file opened: {:?}", audio_path);
    let source = Decoder::new(BufReader::new(file)).map_err(|e| format!("Failed to decode audio file {:?}: {}", audio_path, e))?;
    ...
    // --- Game State & RNG ---
    let mut game_state = initialize_game_state(initial_window_size.width as f32, initial_window_size.height as f32);
    ...
    let quad_vertices: [Vertex; 4] = [
        Vertex { pos: [-0.5, -0.5], tex_coord: [0.0, 0.0] }, Vertex { pos: [ 0.5, -0.5], tex_coord: [1.0, 0.0] },
        Vertex { pos: [ 0.5,  0.5], tex_coord: [1.0, 1.0] }, Vertex { pos: [-0.5,  0.5], tex_coord: [0.0, 1.0] },
    ];
    let vertex_buffer_size = (quad_vertices.len() * mem::size_of::<Vertex>()) as vk::DeviceSize;
    let mut vertex_buffer = base.create_buffer(vertex_buffer_size, vk::BufferUsageFlags::VERTEX_BUFFER, vk::MemoryPropertyFlags::HOST_VISIBLE | vk::MemoryPropertyFlags::HOST_COHERENT)?;
    ...
    let mut index_buffer = base.create_buffer(index_buffer_size, vk::BufferUsageFlags::INDEX_BUFFER, vk::MemoryPropertyFlags::HOST_VISIBLE | vk::MemoryPropertyFlags::HOST_COHERENT)?;
    base.update_buffer(&index_buffer, &quad_indices)?;
    let ubo_size = mem::size_of::<UniformBufferObject>() as vk::DeviceSize;
    let mut projection_ubo = base.create_buffer(ubo_size, vk::BufferUsageFlags::UNIFORM_BUFFER, vk::MemoryPropertyFlags::HOST_VISIBLE | vk::MemoryPropertyFlags::HOST_COHERENT)?;
    ...
    let descriptor_set_layout = unsafe { base.device.create_descriptor_set_layout(&dsl_create_info, None)? };
    let pool_sizes = [
    ...
    let pool_create_info = vk::DescriptorPoolCreateInfo::default().pool_sizes(&pool_sizes).max_sets(1);
    let descriptor_pool = unsafe { base.device.create_descriptor_pool(&pool_create_info, None)? };
    let desc_alloc_info = vk::DescriptorSetAllocateInfo::default().descriptor_pool(descriptor_pool).set_layouts(std::slice::from_ref(&descriptor_set_layout));
    ...
    unsafe { base.device.update_descriptor_sets(&[write_ubo, write_sampler], &[]) };
    let push_constant_ranges = [vk::PushConstantRange { stage_flags: vk::ShaderStageFlags::VERTEX | vk::ShaderStageFlags::FRAGMENT, offset: 0, size: mem::size_of::<PushConstantData>() as u32 }];
    let pipeline_layout_create_info = vk::PipelineLayoutCreateInfo::default().set_layouts(std::slice::from_ref(&descriptor_set_layout)).push_constant_ranges(&push_constant_ranges);
    let pipeline_layout = unsafe { base.device.create_pipeline_layout(&pipeline_layout_create_info, None)? };
    ...
    let vertex_input_state = vk::PipelineVertexInputStateCreateInfo::default().vertex_binding_descriptions(&binding_descriptions).vertex_attribute_descriptions(&attribute_descriptions);
    ...
    let color_blend_attachment = vk::PipelineColorBlendAttachmentState::default().color_write_mask(vk::ColorComponentFlags::RGBA).blend_enable(true).src_color_blend_factor(vk::BlendFactor::SRC_ALPHA).dst_color_blend_factor(vk::BlendFactor::ONE_MINUS_SRC_ALPHA).color_blend_op(vk::BlendOp::ADD).src_alpha_blend_factor(vk::BlendFactor::ONE).dst_alpha_blend_factor(vk::BlendFactor::ZERO).alpha_blend_op(vk::BlendOp::ADD);
    let color_blend_state = vk::PipelineColorBlendStateCreateInfo::default().logic_op_enable(false).attachments(std::slice::from_ref(&color_blend_attachment));
    let depth_stencil_state = vk::PipelineDepthStencilStateCreateInfo::default().depth_test_enable(false).depth_write_enable(false);
    let dynamic_states = [vk::DynamicState::VIEWPORT, vk::DynamicState::SCISSOR];
    let dynamic_state_info = vk::PipelineDynamicStateCreateInfo::default().dynamic_states(&dynamic_states);
    let pipeline_info = vk::GraphicsPipelineCreateInfo::default().stages(&shader_stage_create_infos).vertex_input_state(&vertex_input_state).input_assembly_state(&input_assembly_state).viewport_state(&viewport_state).rasterization_state(&rasterization_state).multisample_state(&multisample_state).color_blend_state(&color_blend_state).depth_stencil_state(&depth_stencil_state).layout(pipeline_layout).render_pass(base.render_pass).subpass(0).dynamic_state(&dynamic_state_info);
    let graphics_pipeline = unsafe { base.device.create_graphics_pipelines(vk::PipelineCache::null(), &[pipeline_info], None).map_err(|(_, err)| err)?[0] };
    unsafe {
    ...
    let projection_matrix = ortho(0.0, game_state.window_size.0, 0.0, game_state.window_size.1, 1.0, -1.0);
    let ubo = UniformBufferObject { projection: projection_matrix };
            ...
            Event::AboutToWait => {
                if resize_needed {
                     ...
                     if let Err(e) = base.update_buffer(&projection_ubo, &[ubo]) {
                         ...
                         info!("Projection matrix updated for new size: {:?}", game_state.window_size);
                     }
                     resize_needed = false;
                }
                ...
                if let Some(fps) = fps_counter.update() {
                    base.window.set_title(&format!("Ash RITG | BPM: {} | FPS: {}", SONG_BPM, fps));
                }
                ...
                match base.draw_frame(|device, cmd_buf| {
                    unsafe {
                         ...
                         device.cmd_bind_descriptor_sets(cmd_buf, vk::PipelineBindPoint::GRAPHICS, pipeline_layout, 0, &[descriptor_set], &[]);
                         ...
                         let uv_width = 1.0 / 4.0; let uv_x_start = frame_index as f32 * uv_width;
                         let uv_offset = [uv_x_start, 0.0]; let uv_scale = [uv_width, 1.0];

                         let now_for_flash = Instant::now();
                         for target in &targets_clone {
                             let current_tint = flash_states_clone.get(&target.direction).filter(|flash| now_for_flash < flash.end_time).map_or(TARGET_TINT, |flash| flash.color);
                             ...
                             let model_matrix = Matrix4::from_translation(Vector3::new(target.x, target.y, 0.0)) * Matrix4::from_angle_z(rotation_angle) * Matrix4::from_nonuniform_scale(TARGET_SIZE, TARGET_SIZE, 1.0);
                             let push_data = PushConstantData { model: model_matrix, color: current_tint, uv_offset, uv_scale };
                             let push_data_bytes = std::slice::from_raw_parts(&push_data as *const _ as *const u8, mem::size_of::<PushConstantData>());
                             device.cmd_push_constants(cmd_buf, pipeline_layout, vk::ShaderStageFlags::VERTEX | vk::ShaderStageFlags::FRAGMENT, 0, push_data_bytes);
                             device.cmd_draw_indexed(cmd_buf, quad_indices.len() as u32, 1, 0, 0, 0);
                         }
                         for column in arrows_clone.values() {
                             for arrow in column {
                                 let arrow_tint = match arrow.note_type { NoteType::Quarter => ARROW_TINT_QUARTER, NoteType::Eighth => ARROW_TINT_EIGHTH, NoteType::Sixteenth => ARROW_TINT_SIXTEENTH };
                                 let rotation_angle = match arrow.direction {
                                    ArrowDirection::Down => Rad(0.0),
                                    ArrowDirection::Left => Rad(PI / 2.0),
                                    ArrowDirection::Up => Rad(PI),
                                    ArrowDirection::Right => Rad(-PI / 2.0) };
                                 let model_matrix = Matrix4::from_translation(Vector3::new(arrow.x, arrow.y, 0.0)) * Matrix4::from_angle_z(rotation_angle) * Matrix4::from_nonuniform_scale(ARROW_SIZE, ARROW_SIZE, 1.0);
                                 let push_data = PushConstantData { model: model_matrix, color: arrow_tint, uv_offset, uv_scale };
                                 let push_data_bytes = std::slice::from_raw_parts(&push_data as *const _ as *const u8, mem::size_of::<PushConstantData>());
                                 device.cmd_push_constants(cmd_buf, pipeline_layout, vk::ShaderStageFlags::VERTEX | vk::ShaderStageFlags::FRAGMENT, 0, push_data_bytes);
                                 device.cmd_draw_indexed(cmd_buf, quad_indices.len() as u32, 1, 0, 0, 0);
                            }
                    ...
                    Ok(needs_resize_result) => { if needs_resize_result { resize_needed = true; } }
...
fn initialize_game_state(win_w: f32, win_h: f32) -> GameState {
     info!("Initializing game state for window size: {}x{}", win_w, win_h);
    let center_x = win_w / 2.0; let target_spacing = ARROW_SIZE * 1.2;
    ...
    let targets = ARROW_DIRECTIONS.iter().enumerate().map(|(i, &dir)| TargetInfo { x: start_x_slot_center + i as f32 * target_spacing, y: TARGET_Y_POS, direction: dir }).collect();
    let mut arrows = HashMap::new(); for dir in ARROW_DIRECTIONS.iter() { arrows.insert(*dir, Vec::new()); }
    ...
    info!("Audio Sync Offset: {} ms -> Initial Beat: {:.4}", AUDIO_SYNC_OFFSET_MS, initial_beat);
    let initial_last_spawned_16th_index = (initial_beat * 4.0 - 1.0).floor() as i32;
     info!("Initial last spawned 16th index: {}", initial_last_spawned_16th_index);
     // Initialize with default ModifiersState
    GameState { targets, arrows, pressed_keys: HashSet::new(), last_spawned_16th_index: initial_last_spawned_16th_index, last_spawned_direction: None, current_beat: initial_beat, window_size: (win_w, win_h), flash_states: HashMap::new(), modifiers: ModifiersState::default() }
}
              ...
              if let winit::keyboard::Key::Named(named_key) = key_event.logical_key {
                 ...
                 if let Some(keycode) = virtual_keycode {
                     match key_event.state {
                         ElementState::Pressed => { if state.pressed_keys.insert(keycode) { check_hits_on_press(state, keycode); } }
                         ElementState::Released => { state.pressed_keys.remove(&keycode); }
                     }
                 } else if key_event.physical_key == PhysicalKey::Code(KeyCode::Escape) && key_event.state == ElementState::Pressed { elwp.exit(); }
             } else if key_event.physical_key == PhysicalKey::Code(KeyCode::Escape) && key_event.state == ElementState::Pressed { elwp.exit(); }
        }
        _ => {}
    }
...
fn key_to_virtual_keycode(key: winit::keyboard::NamedKey) -> Option<VirtualKeyCode> {
    ...
    match key { ArrowLeft => Some(VirtualKeyCode::Left), ArrowDown => Some(VirtualKeyCode::Down), ArrowUp => Some(VirtualKeyCode::Up), ArrowRight => Some(VirtualKeyCode::Right), Escape => Some(VirtualKeyCode::Escape), _ => None }
}

fn check_hits_on_press(state: &mut GameState, keycode: VirtualKeyCode) {
    let direction = match keycode { VirtualKeyCode::Left => Some(ArrowDirection::Left), VirtualKeyCode::Down => Some(ArrowDirection::Down), VirtualKeyCode::Up => Some(ArrowDirection::Up), VirtualKeyCode::Right => Some(ArrowDirection::Right), _ => None };
    if let Some(dir) = direction {
        if let Some(column_arrows) = state.arrows.get_mut(&dir) {
            let current_beat = state.current_beat; let seconds_per_beat = 60.0 / SONG_BPM;
            let mut best_hit_idx: Option<usize> = None; let mut min_time_diff_ms = f32::INFINITY;
            for (idx, arrow) in column_arrows.iter().enumerate() {
                let beat_diff = current_beat - arrow.target_beat; let time_diff_ms = beat_diff * seconds_per_beat * 1000.0; let abs_time_diff_ms = time_diff_ms.abs();
                if abs_time_diff_ms <= MAX_HIT_WINDOW_MS { if abs_time_diff_ms < min_time_diff_ms { min_time_diff_ms = abs_time_diff_ms; best_hit_idx = Some(idx); } }
            }
            if let Some(idx) = best_hit_idx {
                let judgment = if min_time_diff_ms <= W1_WINDOW_MS { Judgment::W1 } else if min_time_diff_ms <= W2_WINDOW_MS { Judgment::W2 } else if min_time_diff_ms <= W3_WINDOW_MS { Judgment::W3 } else { Judgment::W4 };
                let hit_arrow = &column_arrows[idx];
                let time_diff_for_log = (current_beat - hit_arrow.target_beat) * seconds_per_beat * 1000.0;
                info!("HIT! {:?} {:?}. Time Diff: {:.1}ms -> {:?}", dir, hit_arrow.note_type, time_diff_for_log, judgment);
                let flash_color = match judgment { Judgment::W1 => FLASH_COLOR_W1, Judgment::W2 => FLASH_COLOR_W2, Judgment::W3 => FLASH_COLOR_W3, Judgment::W4 => FLASH_COLOR_W4, Judgment::Miss => unreachable!() };
                let flash_end_time = Instant::now() + FLASH_DURATION;
                state.flash_states.insert(dir, FlashState { color: flash_color, end_time: flash_end_time });
                column_arrows.remove(idx);
            } else { debug!("Input {:?} registered, but no arrow within hit window.", keycode); }
        }
    }
}

fn update_game_state(state: &mut GameState, dt: f32, rng: &mut impl Rng) {
    let beat_delta = dt * (SONG_BPM / 60.0); state.current_beat += beat_delta;
    ...
    if target_16th_index > state.last_spawned_16th_index {
        for i in (state.last_spawned_16th_index + 1)..=target_16th_index {
            ...
            let note_type = match i % 4 { 0 => NoteType::Quarter, 2 => NoteType::Eighth, _ => NoteType::Sixteenth };
            let should_spawn = match DIFFICULTY {
                0 => note_type == NoteType::Quarter,
                1 => note_type == NoteType::Quarter || (note_type == NoteType::Eighth && rng.random::<bool>()),
            ...
            if beats_remaining <= 0.0 { continue; }
            ...
            if spawn_y <= TARGET_Y_POS + (ARROW_SIZE * 0.5) { continue; }

            let dir: ArrowDirection;
            if DIFFICULTY >= 3 && state.last_spawned_direction.is_some() {
                let mut available_dirs: Vec<ArrowDirection> = ARROW_DIRECTIONS.iter().copied().filter(|&d| Some(d) != state.last_spawned_direction).collect();
                if available_dirs.is_empty() { available_dirs = ARROW_DIRECTIONS.to_vec(); }
                dir = *available_dirs.choose(rng).unwrap_or(&ARROW_DIRECTIONS[0]);
            } else {
                dir = ARROW_DIRECTIONS[rng.random_range(0..ARROW_DIRECTIONS.len())];
            }
            let target_x = state.targets.iter().find(|t| t.direction == dir).map(|t| t.x).unwrap_or(state.window_size.0 / 2.0);
            if let Some(column_arrows) = state.arrows.get_mut(&dir) {
                column_arrows.push(Arrow { x: target_x, y: spawn_y, direction: dir, note_type, target_beat });
    ...
    for column_arrows in state.arrows.values_mut() { for arrow in column_arrows.iter_mut() { arrow.y -= ARROW_SPEED * dt; } }
    ...
    for (_dir, column_arrows) in state.arrows.iter_mut() {
        column_arrows.retain(|arrow| {
            if state.current_beat > arrow.target_beat + miss_window_beats {
                 info!("MISSED! {:?} {:?} (Target Beat: {:.2}, Current: {:.2}, Diff: {:.1}ms)", arrow.direction, arrow.note_type, arrow.target_beat, state.current_beat, (state.current_beat - arrow.target_beat - miss_window_beats) * seconds_per_beat * 1000.0);
                false
            } else { true }
        });
    }

    let now_for_cleanup = Instant::now(); state.flash_states.retain(|_dir, flash| now_for_cleanup < flash.end_time);
}
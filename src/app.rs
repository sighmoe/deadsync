...
impl App {
    pub fn new(event_loop: &EventLoop<()>) -> Result<Self, Box<dyn Error>> {
        ...
        let renderer = Renderer::new(&vulkan_base, (initial_surface_resolution.width as f32, initial_surface_resolution.height as f32))?;
    ...
    fn try_process_pending_resize(&mut self) -> Result<(), Box<dyn Error>> {
        if let Some((target_size, last_event_time)) = self.pending_resize {
            if Instant::now().duration_since(last_event_time) >= RESIZE_DEBOUNCE_DURATION {
                info!("Debounce time elapsed, processing resize to {:?}.", target_size);
    ...
    fn handle_window_event(&mut self, event: WindowEvent, _elwt: &winit::event_loop::EventLoopWindowTarget<()>) {
        match event {
            ...
            WindowEvent::Resized(new_size) => {
                trace!("Window resized event (raw): {:?}, updating pending resize.", new_size);
    ...
    fn handle_keyboard_input(&mut self, key_event: KeyEvent) {
        ...
        let requested_state = match self.current_app_state {
            ...
            AppState::Options => { 
                options::handle_input(&key_event, &mut self.options_state)
            }
            AppState::Gameplay => {
    ...
    fn transition_state(&mut self, new_state: AppState) {
        ...
        match new_state {
            ...
            AppState::Gameplay => {
                ...
                let window_size_f32 = (self.vulkan_base.surface_resolution.width as f32, self.vulkan_base.surface_resolution.height as f32);
    ...
    fn update(&mut self, dt: f32) {
       ...
       if let Some(fps) = self.fps_counter.update() {
            let title_suffix = match self.current_app_state {
                AppState::Gameplay => format!("Gameplay | FPS: {} | Beat: {:.2}", fps, self.game_state.as_ref().map_or(0.0, |gs| gs.current_beat)),
            ...
            self.vulkan_base.window.set_title(&format!("{} | {}", config::WINDOW_TITLE, title_suffix));
       }
        trace!("Update End");
   }

    fn handle_actual_resize(&mut self, target_size: PhysicalSize<u32>) -> Result<(), Box<dyn Error>> {
        info!("Handling actual resize to physical size: {:?}", target_size);
        
        if target_size.width == 0 || target_size.height == 0 {
            info!("Window is minimized or zero size ({:?}). Deferring Vulkan resize further.", target_size);
        ...
        // Attempt to rebuild
        self.vulkan_base.rebuild_swapchain_resources(target_size.width, target_size.height)?;
        ...
        let new_vulkan_size_f32 = (new_vulkan_surface_extent.width as f32, new_vulkan_surface_extent.height as f32);

        self.renderer.update_projection_matrix(&self.vulkan_base, new_vulkan_size_f32)?;
        
        if let Some(ref mut gs) = self.game_state {
            gs.window_size = new_vulkan_size_f32;
        }

        info!("Actual resize handling complete. New Vulkan surface resolution: {:?}", new_vulkan_surface_extent);
        Ok(())
     }
    ...
    fn render(&mut self) -> Result<bool, vk::Result> { 
        ...
        let draw_result = self.vulkan_base.draw_frame(|device, cmd_buf| {
             ...
             self.renderer.begin_frame(device, cmd_buf, current_surface_resolution); 

             match self.current_app_state {
                 AppState::Menu => {
                     menu::draw(&self.renderer, &self.menu_state, &self.asset_manager, device, cmd_buf);
                 }
                 AppState::SelectMusic => { 
                     select_music::draw(&self.renderer, &self.select_music_state, &self.asset_manager, device, cmd_buf);
                 }
                 AppState::Options => { 
                     options::draw(&self.renderer, &self.options_state, &self.asset_manager, device, cmd_buf);
                 }
                 AppState::Gameplay => {

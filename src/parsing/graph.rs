...
fn calculate_gradient_color(
    y_in_graph_area: u32,
    image_height: u32,
    bottom_rgb: [u8; 3],
    top_rgb: [u8; 3],
) -> [u8; 3] {
    let frac = if image_height <= 1 {
        0.0
    } else {
        y_in_graph_area as f64 / (image_height - 1) as f64
...
fn calculate_bar_heights_pixels(
    measure_nps_vec: &[f64],
    max_nps: f64,
    image_height: u32,
) -> Vec<f64> {
    if max_nps <= 0.0 || image_height == 0 {
        // Prevent division by zero or no height
...
pub fn generate_density_graph_rgba(
    ...
    let measure_pixel_width = if num_measures > 0 {
        IMAGE_WIDTH as f64 / num_measures as f64
    } else {
        IMAGE_WIDTH as f64
    };
    ...
    for x_img in 0..IMAGE_WIDTH {
        ...
        let frac_within_measure_width = if measure_pixel_width > 0.0 {
            (x_img as f64 - (current_measure_idx as f64 * measure_pixel_width))
                / measure_pixel_width
        } else {
        ...
        let current_bar_height_at_x_px =
            h_start_px + frac_within_measure_width * (h_end_px - h_start_px);
        ...
        for y_img in y_img_bar_top_inclusive..IMAGE_HEIGHT {
            ...
            img_buffer_rgba[idx..idx + 4].copy_from_slice(&[
                rgb_color[0],
                rgb_color[1],
                rgb_color[2],
                255,

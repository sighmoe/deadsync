...
impl DescriptorSetId {
    pub fn from_judgment(judgment: Judgment) -> Option<Self> {
        match judgment {
            Judgment::W1 => Some(Self::ExplosionW1),
            Judgment::W2 => Some(Self::ExplosionW2),
...
impl Renderer {
    pub fn new(base: &VulkanBase, initial_window_size: (f32, f32)) -> Result<Self, Box<dyn Error>> {
        ...
        // Increased MAX_SETS for new explosion textures
        const MAX_SETS: u32 = 14; // 9 original + 5 explosions
        let pool_sizes = [
        ...
        let frag_shader_module = {
            let mut file = std::io::Cursor::new(&include_bytes!("../../shaders/msdf_frag.spv")[..]);
            let code = read_spv(&mut file)?;
            let info = vk::ShaderModuleCreateInfo::default().code(&code);
            unsafe { base.device.create_shader_module(&info, None)? }
        };
    ...
    // ... (window_size, update_projection_matrix, update_texture_descriptor, begin_frame, draw_quad, draw_text, destroy methods remain mostly the same) ...
    // Make sure they are compatible with the new structures if any direct usage existed.
    // No direct changes needed in these methods for this specific feature, as DescriptorSetId is an enum.
    pub fn window_size(&self) -> (f32, f32) { self.current_window_size }

    pub fn update_projection_matrix( &mut self, base: &VulkanBase, window_size: (f32, f32), ) -> Result<(), Box<dyn Error>> {
    ...
    pub fn update_texture_descriptor( &self, device: &Device, set_id: DescriptorSetId, texture: &TextureResource, ) {
        let descriptor_set = self .descriptor_sets .get(&set_id) .unwrap_or_else(|| panic!("Invalid DescriptorSetId provided for update: {:?}", set_id));
    ...
    pub fn begin_frame( &self, device: &Device, cmd_buf: vk::CommandBuffer, surface_extent: vk::Extent2D, ) {
        unsafe {
    ...
    pub fn draw_text( &self, device: &Device, cmd_buf: vk::CommandBuffer, font: &Font, text: &str, mut pen_x: f32, pen_y: f32, color: [f32; 4], scale: f32, letter_spacing_factor: Option<f32>, ) {
        let start_pen_x = pen_x;
        let actual_letter_spacing_factor = letter_spacing_factor.unwrap_or(1.0);
        for char_code in text.chars() {
            if char_code == '\n' { pen_x = start_pen_x; debug!("Newline in draw_text. Resetting pen_x. Caller handles pen_y advance."); continue; }
            let advance_amount;
            if char_code == ' ' { advance_amount = font.space_width * scale * actual_letter_spacing_factor; pen_x += advance_amount; continue; }
            if let Some(glyph_info) = font.get_glyph(char_code) {
                ...
                if char_code == 'A' { debug!("DRAW_TEXT 'A': scale: {:.2}", scale); debug!(" GlyphInfo: plane_left={:.2}, plane_bottom={:.2}, plane_right={:.2}, plane_top={:.2}, advance={:.2}", glyph_info.plane_left, glyph_info.plane_bottom, glyph_info.plane_right, glyph_info.plane_top, glyph_info.advance); debug!( " GlyphInfo UVs: u0={:.3}, v0={:.3}, u1={:.3}, v1={:.3}", glyph_info.u0, glyph_info.v0, glyph_info.u1, glyph_info.v1 ); debug!( " Calculated quad_width={:.2}, quad_height={:.2}", quad_width, quad_height ); }
                if quad_width <= 0.0 || quad_height <= 0.0 { advance_amount = glyph_info.advance * scale * actual_letter_spacing_factor; pen_x += advance_amount; continue; }
                ...
                if char_code == 'A' { debug!(" Pen_x={:.2}, Pen_y (baseline)={:.2}", pen_x, pen_y); debug!( " Quad visual_left_x={:.2}, actual_top_y={:.2}", quad_visual_left_x, quad_actual_top_y ); debug!( " Quad center_x={:.2}, center_y={:.2}", quad_center_x, quad_center_y ); debug!( " UV offset=[{:.3}, {:.3}], UV scale=[{:.3}, {:.3}]", glyph_info.u0, glyph_info.v0, glyph_info.u1 - glyph_info.u0, glyph_info.v1 - glyph_info.v0 ); }
                let model_matrix = Matrix4::from_translation(Vector3::new(quad_center_x, quad_center_y, 0.0)) * Matrix4::from_nonuniform_scale(quad_width, quad_height, 1.0);
                if char_code == 'A' { debug!(" Model Matrix for 'A': {:?}", model_matrix); }
                let uv_offset_vals = [glyph_info.u0, glyph_info.v0];
                let uv_scale_vals = [glyph_info.u1 - glyph_info.u0, glyph_info.v1 - glyph_info.v0];
                if char_code == 'A' || text.starts_with("Scaled Text") { debug!( "DRAW_TEXT (char: {}): px_range for push_data: {:.2}", char_code, font.metrics.msdf_pixel_range ); }
                let push_data = PushConstantData { model: model_matrix, color, uv_offset: uv_offset_vals, uv_scale: uv_scale_vals, px_range: font.metrics.msdf_pixel_range, };
                unsafe {
                    let descriptor_set = self .descriptor_sets .get(&font.descriptor_set_id) .unwrap_or_else(|| { panic!("Font descriptor set {:?} not found", font.descriptor_set_id) });
                    device.cmd_bind_descriptor_sets( cmd_buf, vk::PipelineBindPoint::GRAPHICS, self.main_pipeline_layout, 0, &[*descriptor_set], &[], );
                    let push_data_bytes = std::slice::from_raw_parts( &push_data as *const _ as *const u8, std::mem::size_of::<PushConstantData>(), );
                    device.cmd_push_constants( cmd_buf, self.main_pipeline_layout, vk::ShaderStageFlags::VERTEX | vk::ShaderStageFlags::FRAGMENT, 0, push_data_bytes, );
                    device.cmd_draw_indexed(cmd_buf, self.quad_index_count, 1, 0, 0, 0);
                }
                advance_amount = glyph_info.advance * scale * actual_letter_spacing_factor; pen_x += advance_amount;
            } else {
                warn!( "Glyph for '{}' not found in MSDF font. Advancing by space width.", char_code );
                advance_amount = font.space_width * scale * actual_letter_spacing_factor; pen_x += advance_amount;
            }
        }
    }
    ...
    pub fn destroy(&mut self, device: &Device) {
        ...
        unsafe {
            ...
            if self.main_pipeline != vk::Pipeline::null() { device.destroy_pipeline(self.main_pipeline, None); }
            if self.main_pipeline_layout != vk::PipelineLayout::null() { device.destroy_pipeline_layout(self.main_pipeline_layout, None); }
